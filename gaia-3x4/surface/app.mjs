...
import { selectionPatch } from './selection.mjs';
...
const source = await fetch('/examples/canonical.gaia').then(r => { if (!r.ok) throw new Error('Partitura não encontrada.'); return r.text(); });
const first = interpreter.apply(source);
...
let earth;
try { earth = await createEarth($('earth-space'), { onCuePainted: paintCue }); }
...
function fullRecord() {
  return { work: 'GA.IA/3x4: retratos computacionais de uma Terra em trabalho',
    glyphRegistry: GLYPH_REGISTRY, observationEncoding: VISUAL_ENCODING,
    ...
    execution: interpreter.execution, visual: earth?.inspect() ?? null, world: interpreter.world.inspect() };
}
...
function readout() {
  ...
  $('mode').textContent = inspector ? 'INSPECTOR / CANVAS 2D' : 'RETRATO / TERRA SINTÉTICA';
...
function showCue(op, location) {
  const cue = { id: op.id, line: location.line, traceIds: op.traceIds, tick: interpreter.world.tick };
  earth?.setCue(cue);
