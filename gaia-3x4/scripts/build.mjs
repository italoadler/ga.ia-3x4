...
await build({ absWorkingDir: root, entryPoints: ['surface/living-app.mjs'], bundle: true, format: 'esm',
...
await build({ absWorkingDir: root, entryPoints: ['surface/data-app.mjs'], bundle: true, format: 'esm',
  outfile: 'dist/data-instrument.mjs', sourcemap: true, target: ['es2022'], logLevel: 'info',
  minify: process.argv.includes('--minify'), legalComments: 'eof' });
